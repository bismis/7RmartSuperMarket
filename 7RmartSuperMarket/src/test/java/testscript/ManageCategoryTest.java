...
public class ManageCategoryTest extends Base {
	...
	@Test(description = "Verify user can add a new category successfully using valid login credentials", retryAnalyzer = retry.Retry.class) // packagename.classname.classextension
	public void verifyUserCanManageCategory() throws IOException {
		...
		manageCateg.clickOnNewButton().enterCategory().clickOnDiscount().chooseImageFileToUpload().buttonYesTopMenu()
				.buttonYesShowHome().clickSave();

		boolean newAlert = manageCateg.isNewCategAddedAlert();
		Assert.assertTrue(newAlert, constant.CATEGALERT);
	}
